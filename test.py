...
def sort_shapes_by_label(shapes, labels):
    sorted_parts = [Chair() for _ in shapes]

    for i, box in enumerate(shapes):
        box_labels = labels[i]

        for j, cube_label in enumerate(box_labels):
            sorted_parts[i].parts[cube_label].append(box[j])

    return sorted_parts

if __name__ == "__main__":
    grassdata = GRASSDataset('data/Chair', 3)
    
    all_boxes = []
    all_labels = []

    for i in range(len(grassdata)):
        boxes, box_labels = decode_structure(grassdata[i].root)
        cubes = sensiblize(boxes)
        scaled_cubes = [scale(cube, factor=CHAIR_SCALE) for cube in cubes]

        # pdb.set_trace()
        # sample_points = sample_cubes(sensible_boxes, 1000)
        # draw_vertices(sample_points)
        showGenshape(scaled_cubes, box_labels)
        # all_boxes.append(scaled_cubes)
        # all_labels.append(box_labels)
    
    # chairs = sort_shapes_by_label(all_boxes, all_labels)
    # draw_same_part(chairs)