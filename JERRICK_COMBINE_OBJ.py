...
for test_case in selected_test_cases:
    #read json
    with open("data/Chair/" + test_case + "/result.json") as f:
        data = json.load(f)

    #create new folder for each test ID
    if not os.path.isdir('combined_obj_jerrick/' + test_case + '/'):
        os.mkdir('combined_obj_jerrick/' + test_case + '/')
    ...
    for first_children in data[0]['children']:
            ...
            if 'children' in second_children:#if else in case here has no children 
                for third_children in second_children['children']:
                    for objs in third_children['objs']:
                        filepath = "data/Chair/" + test_case + "/objs/"+ objs + ".obj"
        ...
        #export obj file
        scene.export('combined_obj_jerrick/' + test_case + '/' + get_output_obj_name)
